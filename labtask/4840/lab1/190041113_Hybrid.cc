...
NS_LOG_COMPONENT_DEFINE("Hybrid Topology");
...
int main(int argc, char* argv[])
{
    ...
    {
        double theta = (i - 1) * 2.0 * M_PI / (N / 2);
        double x = -2 * radius + radius * cos(theta);
        double y = -2 * radius + radius * sin(theta);
        Ptr<ListPositionAllocator> positionAllocRing = CreateObject<ListPositionAllocator>();
        positionAllocRing->Add(Vector(x, y, 0.0));
        mobility.SetPositionAllocator(positionAllocRing);
        mobility.Install(nodes.Get(i));
    ...
    for (uint32_t i = N / 2; i < N; ++i)
    {
        for (uint32_t j = N / 2; j < N; j++)
        {
            ...
            nodeAdjacencyList.push_back(NodeContainer(nodes.Get(i), nodes.Get(j)));
        }
    }
    ...
    for (uint32_t i = 0; i < deviceAdjacencyList.size(); ++i)
    {
        ...
        mask << "255.255.255."
             << (0xff & (0xff << (uint8_t)std::floor(8 - log2(deviceAdjacencyList.size()))));
    ...
    AnimationInterface anim("190041113_Hybrid.xml");
    ...
    for (uint32_t i = N / 2 + 1; i < N; i++)
    {
        ...
        anim.SetConstantPosition(nodes.Get(i),
                                 -2 * radius + radius * cos(theta),
                                 -2 * radius + radius * sin(theta));
    }
