...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE("Star Topology");
int main(int argc, char* argv[]) {
    ...
    uint32_t links[6][2] = {{0, 1}, {0, 2}, {0, 3}, {1, 0}, {2, 0}, {3, 0}};    
    for (uint32_t i = 1; i <= 3; ++i){
    	devices.Add(p2p.Install(nodes.Get(0), nodes.Get(i)));
    }    
    ...
    // AnimationInterface anim("task01.xml");
    Simulator::Run();
    Simulator::Destroy();
    return 0;
}
