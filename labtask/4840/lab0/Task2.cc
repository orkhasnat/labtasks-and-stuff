...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE("Ring Topology");
int main(int argc, char* argv[]) {
    ...
    uint32_t links[4][2] = {{0, 1}, {1, 2}, {2, 3}, {3, 0}}; 
   
    for (uint32_t i = 0; i <4; ++i){
    	devices.Add(p2p.Install(nodes.Get(i), nodes.Get((i+1)%4)));
    }    
    ...
    // AnimationInterface anim("task02.xml");
    Simulator::Run();
    Simulator::Destroy();
    return 0;
}
