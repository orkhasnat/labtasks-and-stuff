...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if self.walls[nextx][nexty] is False:
                nextState = (nextx, nexty)
                nextVis = copy.deepcopy(currVis)
                for corner in self.corners:
                    if nextState == corner:
                        nextVis[nextState] = True

                successors.append(((nextState, nextVis), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    '''
    the corner thats farthest from our pacman is our goal state in this case and we need to maximize such distance thus we take all the distance and get the max from it.
    '''
...
def foodHeuristic(state, problem):
    ...
    '''
    we get the list of food coords and compute the distnce from pacman to food, need maximum from them to get under estimation.
    '''
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # util.raiseNotDefined()
        '''
        same as FoodHeuristics() as before but we used the minimum distance because we are going iteratively and pacman wants to eat the closest food first.
        Also we are implementing basic ucs and instead of problem.getCostofActions() we are providing the minimum heuristics
        '''

        closest_heuristics = lambda position, foodGrid: min([util.manhattanDistance(position, food) for food in foodGrid.asList()])
            ...
            if currState not in visited:
                for nextstate, action, cost in problem.getSuccessors(currState):
                    fringe.push(
                        ...
                        closest_heuristics(startPosition, food),
                    )
